...
typedef int (*pf_h)(vector<vector <int> > nowmap);
typedef void (*pf_A)(Node& source);
pf_h the_h;
pf_A the_A;
...
void run(string inputpath,string targetpath)
{
...
int main(int argc, char **argv) 
{
    if (argc != 4)
        return -1;
    char *type = argv[1];
    char *start_name = argv[2], *target_name = argv[3];
	string inputpath,targetpath;
	inputpath=start_name;
	targetpath=target_name;
	if((strcmp(type, "A_h1") == 0)&&(strcmp(type, "IDA_h1") == 0))
		the_h = h1;
	if((strcmp(type, "A_h2") == 0)&&(strcmp(type, "IDA_h2") == 0))
		the_h = h2;
	if((strcmp(type, "A_h1") == 0)&&(strcmp(type, "A_h2") == 0))
		the_A = A_star;
	if((strcmp(type, "IDA_h1") == 0)&&(strcmp(type, "IDA_h2") == 0))
		the_A = IDA;
    run(inputpath,targetpath);
    return 0;
}